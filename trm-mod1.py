...
Optimized for 8GB VRAM with Stable Variance Initialization, Deep Supervision,
purely STREAMING datasets, and CHECKPOINT SAVING.
"""
# final big boy script 16 day training
import os
import sys
import json
import math
...
class TRMConfig:
    ...
    # ── Scaled up for the long run ──
    max_steps: int = 50000         # Increased to 50k steps
    save_interval: int = 1000      # Will save a checkpoint every 1000 steps
...
class RMSNorm(nn.Module):
    def __init__(self, dim: int, eps: float = 1e-5):  # Fixed: __init__ instead of _init_
...
class CausalSelfAttention(nn.Module):
    def __init__(self, cfg):  # Fixed: __init__
        super().__init__()  # Fixed: super().__init__()
        ...
        causal_mask = torch.triu(
            torch.full((cfg.max_position_embeddings, cfg.max_position_embeddings), float("-inf")), 
            diagonal=1
        )
...
class TRMForCausalLM(nn.Module):
    def __init__(self, cfg, vocab_size):  # Fixed: __init__
        super().__init__()  # Fixed: super().__init__()
        self.cfg = cfg
...
# ── EMA & Streaming Training Loop ──────────────────────────────────

class EMA:
    def __init__(self, model, decay=0.999):  # Fixed: __init__
        self.decay = decay
    ...
    for i, batch in enumerate(pbar):
        ...
        if (i + 1) % cfg.gradient_accumulation_steps == 0:
            ...
            if optim_step % cfg.save_interval == 0:
                ...
                # 1. Create directory for this specific checkpoint
                save_dir = os.path.join(cfg.output_dir, f"checkpoint-{optim_step}")
                os.makedirs(save_dir, exist_ok=True)
                
                # 2. Save PyTorch dictionary
                ...
                print(f"✅ Weights physically saved to disk: {ckpt_path}")
                print("-" * 30 + "\n")
...
# ── Streaming Dataset & Runner ──────────────────────────────────────

class FinewebEduStreamingDataset(IterableDataset):
    def __init__(self, tokenizer, max_len, subset="sample-10BT", split="train"):  # Fixed: __init__
        super().__init__()  # Fixed: super().__init__()
...
def main():
    ...
    train_ds = FinewebEduStreamingDataset(tokenizer, cfg.max_seq_length, subset=cfg.dataset_subset, split="train")
